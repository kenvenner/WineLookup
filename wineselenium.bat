...
REM
REM Messaging
echo %date% %time% wine.py use selenium to read in new data >> wineselenium.log
REM
...
if errorlevel 1 (
   ...
   echo %date% %time% wine.py run again to do failure 1 >> wineselenium.log
...
if errorlevel 1 (
   TASKKILL /IM chrome.exe /F
   TASKKILL /IM chromedriver.exe /f
   echo %date% %time% wine.py run again to do failure 2 >> wineselenium.log
...
if errorlevel 1 (
   TASKKILL /IM chrome.exe /F
   TASKKILL /IM chromedriver.exe /f
   echo %date% %time% wine.py run again to do failure 3 >> wineselenium.log
...
if errorlevel 1 (
   TASKKILL /IM chrome.exe /F
   TASKKILL /IM chromedriver.exe /f
   echo %date% %time% wine.py not run again to do failure 4 >> wineselenium.log
